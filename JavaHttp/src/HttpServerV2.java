...
public class HttpServerV2 {
    ...
    public void process(Socket clientSocket) {
        try {
            // 1. 读取并解析请求
            HttpRequestV2 request = HttpRequestV2.build(clientSocket.getInputStream());
            System.out.println("request: " + request);
            HttpResponseV2 response = HttpResponseV2.build(clientSocket.getOutputStream());
    ...
    public static void main(String[] args) throws IOException {
        HttpServerV2 server = new HttpServerV2(1234);
        server.start();
    }
}
